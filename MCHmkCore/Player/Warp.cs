...
namespace MCHmk {
    public static class Warp {
        ...
        private static readonly string warpsPath = Path.Combine("extra", "warps.save");
        ...
        public static void SAVE() {
            using (StreamWriter SW = new StreamWriter(warpsPath)) {
                foreach (Wrp warp in Warps) {
                    SW.WriteLine(warp.name + ":" + warp.lvlname + ":" + warp.x.ToString() + ":" + warp.y.ToString() + ":" +
                                 warp.z.ToString() + ":" + warp.rotx.ToString() + ":" + warp.roty.ToString());
                }
                ...
                try {
                    if (TempDeletedWarpsList.Count >= 1) {
                        ...
                        foreach (Wrp BAKwarp in TempDeletedWarpsList) {
                            SW.WriteLine("#" + BAKwarp.name + ":" + BAKwarp.lvlname + ":" + BAKwarp.x.ToString() + ":" + BAKwarp.y.ToString() + ":"
                                         + BAKwarp.z.ToString() + ":" + BAKwarp.rotx.ToString() + ":" + BAKwarp.roty.ToString());
                        }
                    }
                    ...
                    logger.ErrorLog(e);
                    logger.Log("Saving backups of deleted warps failed!");
                }
                ...
                try {
                    if (FailedLoadingWarpsList.Count >= 1) {
                        ...
                        foreach (Wrp FAILwarp in FailedLoadingWarpsList) {
                            SW.WriteLine("#" + FAILwarp.name + ":" + FAILwarp.lvlname + ":" + FAILwarp.x.ToString() + ":" + FAILwarp.y.ToString() +
                                         ":" + FAILwarp.z.ToString() + ":" + FAILwarp.rotx.ToString() + ":" + FAILwarp.roty.ToString());
                        }
                    }
                    ...
                    logger.ErrorLog(e);
                    logger.Log("Saving failed loading warps failed!");
                }
                SW.Dispose();
            }
        }
        ...
        public static void LOAD() {
            if (File.Exists(warpsPath)) {
                using (StreamReader SR = new StreamReader(warpsPath)) {
                    bool failed = false;
                    ...
                    while (SR.EndOfStream == false) {
                        ...
                        if (!line.StartsWith("#") && line.Contains(":")) {
                            ...
                            try {
                                ...
                                logger.Log("Couldn't load a Warp! Look in the 'extra/warps.save' file to see the unloaded warp");
