    ...
    public class Listener {
        ...
        private void ParseCommand(string[] tokens ) {
            ...
            default:
                ...
                Debug.WriteLineIf( Rfc2812Util.IrcTrace.TraceWarning,
                                   "[" + Thread.CurrentThread.Name +"] Listener::ParseCommand() Unknown IRC command=" + tokens[1] );
                break;
            }
        }
        private void ParseReply( string[] tokens ) {
            ...
            case ReplyCode.RPL_CHANNELMODEIS:
                if( OnChannelModeRequest != null ) {
                    try {
                        ...
                        Debug.WriteLineIf( Rfc2812Util.IrcTrace.TraceWarning,
                                           "[" + Thread.CurrentThread.Name +"] Listener::ParseReply() Bad IRC MODE string=" + tokens[0] );
                    }
                }
                break;
            ...
            case ReplyCode.RPL_INVITELIST:
                if( OnChannelList != null ) {
                    OnChannelList( tokens[3], ChannelMode.Invitation, tokens[4], Rfc2812Util.UserInfoFromString(tokens[5]),
                                   Convert.ToInt64(tokens[6]),false );
                }
                break;
            ...
            case ReplyCode.RPL_EXCEPTLIST:
                if( OnChannelList != null ) {
                    OnChannelList( tokens[3], ChannelMode.Exception, tokens[4], Rfc2812Util.UserInfoFromString(tokens[5]),
                                   Convert.ToInt64(tokens[6]),false );
                }
                break;
