    ...
    public class DccChatSession {
        ...
        private void TimerExpired( object state ) {
            if( listening ) {
                Debug.WriteLineIf( DccUtil.DccTrace.TraceInfo,
                                   "[" + Thread.CurrentThread.Name +"] DccChatSession::TimerExpired() Chat session " + this.ToString() + " timed out.");
        ...
        private void Listen() {
            ...
            try {
                ...
                listening = false;
                Debug.WriteLineIf( DccUtil.DccTrace.TraceInfo,
                                   "[" + Thread.CurrentThread.Name +"] DccChatSession::Listen() Remote user connected.");
                ...
                Debug.WriteLineIf( DccUtil.DccTrace.TraceInfo,
                                   "[" + Thread.CurrentThread.Name +"] DccChatSession::Listen() Connection broken");
            }
            finally {
                SendClosedEvent();
            }
        ...
        private void Connect() {
            ...
            try {
                ...
                Debug.WriteLineIf( Rfc2812Util.IrcTrace.TraceWarning,
                                   "[" + Thread.CurrentThread.Name +"] DccChatSession::Connect() exception=" + se );
        ...
        private void ReceiveMessages() {
            ...
            try {
                ...
                while( ( message = reader.ReadLine() ) != null ) {
                    if( OnChatMessageReceived != null ) {
                        ...
                        Debug.WriteLineIf( DccUtil.DccTrace.TraceVerbose,
                                           "[" + Thread.CurrentThread.Name +"] DccChatSession::ReceiveMessages() Session: " + ToString() + " Received: " +
                                           message );
                        Debug.Unindent();
                ...
                Debug.WriteLineIf( DccUtil.DccTrace.TraceWarning,
                                   "[" + Thread.CurrentThread.Name +"] DccChatSession::ReceiveMessages() Thread manually stopped. ");
                //Prevent the exception from being re-thrown in the Listen() method.
                Thread.ResetAbort();
            }
            catch( Exception e) {
                Debug.WriteLineIf( DccUtil.DccTrace.TraceWarning,
                                   "[" + Thread.CurrentThread.Name +"] DccChatSession::ReceiveMessages() exception= "+ e);
            }
            finally {
                CloseClientConnection();
            }
        ...
        public void SendMessage( string text ) {
            if( Connected ) {
                try {
                    ...
                    Debug.WriteLineIf( DccUtil.DccTrace.TraceVerbose,
                                       "[" + Thread.CurrentThread.Name +"] DccChatSession::SendMessage() Sent : " + text + " Size: " + messageBytes.Length );
                }
                catch( Exception e ) {
                    Debug.WriteLineIf( DccUtil.DccTrace.TraceWarning,
                                       "[" + Thread.CurrentThread.Name +"] DccChatSession::SendMessage() " + e );
                }
            }
        }
        /// <summary>
        ...
        public static DccChatSession Request( DccUserInfo dccUserInfo, string listenIPAddress, int listenPort, long timeout ) {
            ...
            if( timeout > 0 ) {
                /*	Timer timer = new Timer(
                		...
                		0);*/
                Debug.WriteLineIf( DccUtil.DccTrace.TraceInfo,
                                   "[" + Thread.CurrentThread.Name +"] DccChatSession::Request timeout thread started");
            }
            return session;
        }
