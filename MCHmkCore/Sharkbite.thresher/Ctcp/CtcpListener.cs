...
namespace Sharkbite.Irc {
    ...
    public class CtcpListener {
        ...
        static CtcpListener() {
            ...
            ctcpRegex = new Regex(":([^ ]+) [A-Z]+ [^:]+:\u0001" + ctcpTypes + "([^\u0001]*)\u0001",
            ...
            if( ctcpTokens != null ) {
                if( ctcpTokens[ Command ].ToUpper( CultureInfo.CurrentCulture ) == CtcpUtil.Ping ) {
                    ...
                    if( IsReply( ctcpTokens ) ) {
                        if( OnCtcpReply != null ) {
                            OnCtcpReply( ctcpTokens[ Command].ToUpper( CultureInfo.CurrentCulture ) ,
                                         Rfc2812Util.UserInfoFromString( ctcpTokens[ Name ] ), ctcpTokens[ Text] );
                        }
                    }
                    else {
                        if( OnCtcpRequest != null ) {
                            OnCtcpRequest( ctcpTokens[ Command].ToUpper( CultureInfo.CurrentCulture ) ,
                                           Rfc2812Util.UserInfoFromString( ctcpTokens[ Name ] ));
                        }
                    }
                }
            }
